...
class Config(object):
    def __init__(self, args):
        ...
        # If using run_parallel script without swarm (only to set different seeds)
        args.seed = args.base + args.inc
    ...
    def get_domain(self, tag, args, path, debug=True):
        print(path)
        if tag in ('MEDEVAC', 'MountainCar', 'RoboToy'):
            obj = utils.dynamic_load(path, tag, load_class=True)
            env = obj(debug=debug, speed=self.speed)
            return env, False, env.action_space.dtype == np.float32

        elif tag == 'SimGlucosediscreteNS-v0':
            import gym
            ...
            register(
                id='SimGlucosediscreteNS-v0',

                entry_point='Environments.SimGlucose.simglucose.envs:NS_T1DSimEnv',
                kwargs={'patient_name': 'adolescent#002', 'speed': self.speed, 'oracle': -1, 'seed': self.seed}
            )
            #     entry_point='Environments.SimGlucose.simglucose.envs:T1DSimEnv_NS',
            #     kwargs={'patient_name': 'adolescent#002', 'speed': self.speed, 'seed': self.seed}
            # )
