...
from Src.CollectData.solver import Solver
...
class Parser(object):
    def __init__(self):
        ...
        parser.add_argument("--max_episodes", default=int(1e4), help="maximum number of training episodes", type=int)
    ...
    def Env_n_Agent_args(self, parser):
        ...
        parser.add_argument("--env_name", default='MountainCar', help="Environment to run the code")
        parser.add_argument("--speed", default=0, help="Rate of nonstationarity", type=int)

    def Main_AC_args(self, parser):
        parser.add_argument("--alpha", default=0.5, help="Mixing ratio for behavior policy", type=float)
        parser.add_argument("--gamma", default=1, help="Discounting factor", type=float)
        parser.add_argument("--actor_lr", default=5e-4, help="Learning rate of actor", type=float)
        parser.add_argument("--state_lr", default=5e-4, help="Learning rate of state features", type=float)
        parser.add_argument("--critic_lr", default=5e-3, help="Learning rate of state features", type=float)
...
def main(mode='train', inc=-1, hyper='default', base=-1):
    ...
    if mode == 'train':
        ...
        solver.collect(max_episodes=int(2e3))
        solver.eval(max_episodes=int(5e2))
